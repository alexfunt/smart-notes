...
class NoteService:
    ...
    async def delete_note(self, note_id: int, user_note_number: int) -> bool:
        user = await self.user_repo.get_by__telegram_id(telegram_id)
        if not user:
            return False
        
        note = await self.note_repo.get_by_note_number(user.id, user_note_number)
        if not note:
            return False
